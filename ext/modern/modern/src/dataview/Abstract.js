...
Ext.define('Ext.dataview.Abstract', {
    ...
    config: {
         ...
         * {@link Ext.dataview.Feladatok Lists}.  When this config is set to `true` each item's
     ...
     * not always the case, especially for the `Ext.Feladatok` subclass. To be clear about
     ...
     *
     * Be aware that the `Ext.Feladatok` subclass can optionally render only some records, in
     ...
     * items). These are often unaligned in `Ext.Feladatok` due to group headers as well as
     ...
     * in a dataview correspond to records (such as group headers in `Ext.Feladatok`). In these
     ...
     * unaligned in `Ext.Feladatok` due to group headers as well as `infinite` mode where not
    ...
    onInnerFocusEnter: function(e) {
        ...
        if (itemCount) {
            ...
            // element as focusable. If not, Feladatok#createItem uses the encapsulating el.
