...
$feladat = new Feladat($db);
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        if ($feladat->create()) {
            echo json_encode(
                array('message' => 'Feladat lértrehozva')
            );
        } else {
            echo json_encode(
                array('message' => 'A feladat nem lett létrehozva')
            );
        ...
        if ($feladat->update()) {
            echo json_encode(
                array('message' => 'Feladat módosítva')
            );
        } else {
            echo json_encode(
                array('message' => 'A feladat nem lett módosítva')
            );
        ...
        if (!isset($_GET['id']))
            ...
            if ($feladat->delete()) {
                echo json_encode(
                    array('message' => 'Feladat törölve')
                );
            } else {
                echo json_encode(
                    array('message' => 'A feladat nem lett törölve')
                );
